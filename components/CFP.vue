<template>
  <section id="cfp" class="container--white">
    <div class="container--fix container--center">
      <h2>Call for paper</h2>
      <p>
        Vous pouvez proposer un sujet jusqu'au <span class="label--text label--green">dimanche 9 décembre 2018</span>, la délibération se fera le <span class="label--text label--green">dimanche 23 décembre 2018</span>.
      </p>
      <p>
        <a class="button--link button--green" href="https://cfp.touraine.tech/speaker" target="_blank">Proposez vos sujets !</a>
      </p>
    </div>
  </section>
</template>

<style scoped>
  p {
    margin-bottom: 4rem;
  }

  .button--link{
    font-size: 24px;
  }
</style>
