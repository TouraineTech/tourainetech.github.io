  ...
  <div class="container--fix">
    ...
    <div class="description--container">
      ...
      <div class="description--link">
        <a :href="sponsor.link" target="_blank">Visiter leur site internet</a>
      </div>
    </div>
...
<script>
  export default {
    validate({store, params}) {
      return store.state.sponsors.filter(sponsor => sponsor.id === params.name).length > 0
    },
    asyncData({store, params}) {
      return {
        sponsor: store.state.sponsors.filter(sponsor => sponsor.id === params.name)[0]
      }
    },
    head() {
      const title = `Touraine Tech 2019 - Merci à ${this.sponsor.name} notre partenaire ${this.sponsor.type}`
      const url = `https://touraine.tech/sponsor/${this.sponsor.id}`
      const image = 'https://touraine.tech' + require(`@/assets/img/${this.sponsor.image}`)
      return {
        titleTemplate: title,
        meta: [
          {hid: 'description', name: 'description', content: this.sponsor.desc},
          {hid: 'ogtitle', property: 'og:title', content: title},
          {hid: 'ogdescription', property: 'og:description', content: this.sponsor.desc},
          ...
          {hid: 'twitterdescription',name: 'twitter:description', content: this.sponsor.desc},
          {hid: 'twitterimage',name: 'twitter:image', content: image},
          {hid: 'twitterimagealt',name: 'twitter:image:alt', content: `Logo ${this.sponsor.name}`},
        ],
      }
    }
  }
