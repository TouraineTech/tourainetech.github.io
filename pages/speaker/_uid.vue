...
export default {
  validate({ store, params }) {
    return (
      store.getters.speakers.filter(speaker => speaker.uid === params.uid)
        .length > 0
    );
  },
  asyncData({ store, params }) {
    return {
      speaker: store.getters.speakers.filter(
        speaker => speaker.uid === params.uid
      )[0]
    };
  },
  head() {
    const title = `Touraine Tech 20${CONFIGURATION.eventEdition} - ${this.speaker.name}`;
    const url = `https://touraine.tech/speaker/${this.speaker.uid}`;
    const image = `${this.speaker.picture}`;
    return {
      titleTemplate: title,
      meta: [
