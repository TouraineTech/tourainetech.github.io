...
const speaker = computed(() =>
  store.speakers.find((s) => s.uid === route.params.uid),
)

if (!speaker.value) {
  throw createError({ statusCode: 404, message: 'Speaker not found' })
}

const bioHTML = computed(() => converter.makeHtml(speaker.value?.bio || ''))

const title = computed(
  () => `Touraine Tech 20${CONFIGURATION.eventEdition} - ${speaker.value?.name}`,
)
const url = computed(() => `https://touraine.tech/speaker/${speaker.value?.uid}`)

useHead({
  title: title.value,
  meta: [
    { hid: 'description', name: 'description', content: speaker.value?.bio },
    { hid: 'ogtitle', property: 'og:title', content: title.value },
    { hid: 'ogdescription', property: 'og:description', content: speaker.value?.bio },
    ...
    { hid: 'twitterdescription', name: 'twitter:description', content: speaker.value?.bio },
  ],
})
</script>

<template>
  <div v-if="speaker" class="container--fix">
    <div class="container--image">
      <img
        :src="`/img/speakers/${speaker.uid}.png`"
        :alt="speaker.uid"
      >
    </div>
    <h1>{{ speaker.name }}</h1>
    <div class="speaker--links">
      <ul>
        ...
        <li v-if="speaker.mastodon">
          <a
            target="_blank"
            :href="`https://${speaker.mastodon.server}/${speaker.mastodon.username}`"
          >
            ...
            {{ speaker.mastodon.username }}@{{ speaker.mastodon.server }}
          </a>
        </li>
